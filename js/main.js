$(document).ready(function () {
	...
	$('.hover_link').on('mouseenter', function () {
		...
		$('.hidden_menu').height($('.result_block').next().height());
	...
	$('.header_menu_item_p:not(.header_menu_item_submenu ul .header_menu_item_p), .mobile_header_menu_a').on('click', function (e) {
	...
	if (basicSlider != null) {
		...
		$(".basic_slider").each(function () {
			...
			var prevArr = $(this).parents('.basic_slider_wrapper').find('.basic_slider_nav--arr_left')[0];

			var nextArr = $(this).parents('.basic_slider_wrapper').find('.basic_slider_nav--arr_right')[0];
			...
			new Swiper($(this)[0], {
				...
				pagination: {
					...
					el: $(this).parents('.basic_slider_wrapper').find('.basic_slider_dots')[0],
...
// }
	...
	if (productSlider != null) {
		...
		var productSingleSliderThumbnails = new Swiper($('.product_slider_thumbnails')[0], {
	...
	$(document).on('click', '.open_modal', function (event) {
		...
		$('.modal_window.active').animate({ opacity: 0, top: 45 + "%" }, 300, function () { $(this).removeClass('active'); });

		if ($(this).hasClass('btn_tooltip')) {

			$("#modal_callback select").val($(this).attr('data-title')).trigger('change');
		...
		if ($(div).hasClass('modal_slider')) {
			...
			if ($(this).attr('data-item') != null) {
				...
				$(div).find('.slick-slider').each(function () {
